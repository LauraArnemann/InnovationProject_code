...
********************************************************************************
// We impute the location of establishments in between patents (based on lcoation of inventor)	
	
	use "${TEMP}/patents`num'.dta", clear 
	...
	* First and last year of patenting activity of the establishment
	...
	*Expand data to all sample years
	expand 51 
	...
	compress
	save "${TEMP}/helper_locations`num'.dta", replace 
...
********************************************************************************
	...
	* Merging based on whether there was RD activity in the state when the establishment was first active 
	use "${TEMP}/helper_locations`num'.dta", clear 
	...
	* This observation is always empty  
	...
	if $gvkey == 1 {
		save "${TEMP}/other_first`num'_gvkey.dta", replace 
}


********************************************************************************
* R&D Credit at other locations based on presence during the time period in which we  
...
might address this in a robustness check   */
	...
	* Only keep multi-state firms 
	...
	merge m:1 assignee_id app_year using "${TEMP}/helper_locations`num'.dta"
	keep if _merge ==3 
	drop _merge 
	...
	* Generate the different variables weighted by the patenters respective inventors 
	...
	bysort assignee_id fips_state other_fips_state: egen other_patents = total(patents`num') 
	...
	*label var other_weighted1 "RD Credit, weighted by Inventors" 
	...
	if $gvkey == 0 {
		save "${TEMP}/other_all`num'.dta", replace 
}
	if $gvkey == 1 {
		save "${TEMP}/other_all`num'_gvkey.dta", replace 
}
...
********************************************************************************
	...
	duplicates drop fips_state assignee_id year other_fips_state, force // This should not delete anything
	...
	* Keep the observations with the largest weights 
	...
	if $gvkey == 0 {
		save "${TEMP}/other_threelargest`num'.dta", replace 
}
	if $gvkey == 1 {
		save "${TEMP}/other_threelargest`num'_gvkey.dta", replace 
}


	* Erasing all the helper files created along the way to keep storage space clean
	erase "${TEMP}/helper_locations`num'.dta"
	erase "${TEMP}/helper_other_v`num'.dta"
	erase "${TEMP}/helper_other_cleaned`num'.dta"
