...
forvalues state = 1(1)2 {	// 1-56
	...
	if `state' == 3 | `state' == 7 | `state' == 14 | `state' == 43 | `state' == 52 {
		...
		*Restrict filesize, run code by state
		merge m:1 assignee_id using "${TEMP}/final_cz_${dataset}_preaggregate_`state'.dta", nogen keep(3)
