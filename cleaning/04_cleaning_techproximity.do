...
use "${TEMP}/final_cz_${dataset}_preaggregate.dta", clear
...
foreach cz of local all_cz {
	...
	save "${TEMP}/final_cz_${dataset}_preaggregate_`cz'.dta", replace
}
...
foreach cz of local all_cz {	
	...
	*Restrict filesize, run code by state
	merge m:1 assignee_id using "${TEMP}/final_cz_${dataset}_preaggregate_`cz'.dta", nogen keep(3)
...
local tmax=r(max)
forv y=`tmin'(1)`tmax' {
	use "${TEMP}/final_cz_${dataset}_preaggregate.dta" if year==`y', clear
	rename assignee_id assignee_id_
	save "${TEMP}/final_cz_${dataset}_preaggregate`y'.dta", replace
}

*Spillover variable
*- Re-normalized variable: 1/100
*- Sum over all other assignees
*[- Could think about weighting with average inventor count as replacement for R&D]
forv y=`tmin'(1)`tmax' {
	use "${TEMP}/spill_tmp", clear
	merge m:1 assignee_id_ using "${TEMP}/final_cz_${dataset}_preaggregate`y'.dta", nogen keep(3)
	
	foreach var in "tec covtec maltec malcovtec" {
		egen spill`var'=sum(`var'*(1/100)*(assignee_id!=assignee_id_)), by(assignee_id)
		egen spill`var'_cz=sum(`var'*(1/100)*(assignee_id!=assignee_id_)), by(assignee_id czone)	// TEC for all firms within CZ
		drop `var'
	}
...
forv y=`tmin'(1)`tmax' {
	erase "${TEMP}/final_cz_${dataset}_preaggregate`y'.dta"
	erase "${TEMP}/spill_output`y'.dta"
}
