...
* Generate a helper file matching gvkey to assignee_id 

use "$inventordata", clear 
drop if missing(assignee_id)
drop if missing(gvkey)
keep assignee_id gvkey app_year 
duplicates tag assignee_id gvkey app_year, gen(dup)
drop if dup!=0 
drop dup
duplicates tag assignee_id app_year, gen(dup)
drop if dup!=0 
drop dup
* Honestly I don't know where this comes from, but only drops 271 observations, so should be fine
save "${TEMP}/helper_gvkey_assignee.dta", replace 
...
use "${TEMP}/other_threelargest_3_4_gvkey.dta", clear 
...
merge m:1 assignee_id app_year using "${TEMP}/helper_gvkey_assignee.dta", keepusing(gvkey)
drop if _merge==2 

* Fill in gvkey 
gsort assignee_id - _merge 
bysort assignee_id: replace gvkey=gvkey[_n-1] if missing(gvkey) & gvkey[_n-1]!=""
...
merge m:1 fips_state gvkey year using "${TEMP}/other_threelargest_3_$dataset_treated_gvkey.dta"
...
/*
    Result                           # of obs.
    -----------------------------------------
    not matched                     2,008,167
        from master                 1,951,387  (_merge==1)
        from using                     56,780  (_merge==2)

    matched                           113,338  (_merge==3)
    -----------------------------------------
*/
