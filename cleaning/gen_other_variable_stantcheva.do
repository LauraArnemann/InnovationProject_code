...
bysort fips_state assignee_id: egen total_patents = total(patents1)
...
save "${TEMP}/helper_dataset_stantcheva.dta", replace 
...
use "${TEMP}/helper_dataset_stantcheva.dta", clear 
...
save "${TEMP}/other_first_stantcheva.dta", replace 
...
merge m:1 assignee_id app_year using "${TEMP}/helper_dataset_stantcheva.dta"
keep if _merge ==3 
drop _merge 
...
save "${TEMP}/helper_other_v_stantcheva.dta", replace 


	
use "${TEMP}/helper_other_v_stantcheva.dta", clear 
...
save "${TEMP}/helper_other_cleaned_stantcheva.dta", replace 
...
save "${TEMP}/other_all_stantcheva.dta", replace 
...
use "${TEMP}/helper_other_cleaned_stantcheva.dta", clear 
...
keep if count ==1 
save "${TEMP}/other_threelargest_stantcheva.dta", replace 


* Erasing all the helper files I created along the way to keep storage space clean


erase "${TEMP}/helper_dataset_stantcheva.dta"
erase "${TEMP}/helper_other_v_stantcheva.dta"
erase  "${TEMP}/helper_other_cleaned_stantcheva.dta"
