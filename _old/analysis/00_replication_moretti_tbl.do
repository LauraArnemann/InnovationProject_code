...
count if state_fips_inventor == . // 536,688 // Laura: 536,181
count if state_inventor == "" //  536,117 // Laura: 536,118
...
collapse (first) residence_state assignee_id (sum) patent_share, by(inventor_id app_year)
...
gen companymigration = 0 if migration==0
replace companymigration=1 if residence_state!=origin_state & residence_state!="" & origin_state!="" & assignee_id==assignee_id[_n-1]

*17.226 inventors which migrate in their company
...
* Number of scientists not moving, only scientists moving which stayed in their company
gen inventor_id_stay_c = inventor_id if companymigration == 0 // could also do != 1; how to deal with missings?
bysort residence_state app_year: egen pop_all_stayers_c = count(inventor_id_stay) 
	gen superstar_stay_c = superstar if companymigration == 0
bysort residence_state app_year: egen pop_star95_stayers_c = sum(superstar_stay_c)
...
//HAS TO BE CALCULATED FOR EACH ORIGIN-RESIDENCE PAIR
	gen inventor_id_move_c = inventor_id if companymigration == 1
bysort residence_state origin_state app_year: egen outflow_all_c = count(inventor_id_move_c)
	replace outflow_all_c = pop_all_stayers_c if residence_state == origin_state	
			//Moretti/Wilson do that
	
	gen superstar_move_c = superstar if companymigration == 1
bysort residence_state origin_state app_year: egen outflow95_c = sum(superstar_move_c) 
	replace outflow95_c = pop_star95_stayers_c if residence_state == origin_state	
...
gen oddsratio_all_c = outflow_all_c / pop_all_stayers_c
 gen log_oddsratio_all_c = log(oddsratio_all_c)
gen oddsratio95_c = outflow95_c / pop_star95_stayers_c
	gen log_oddsratio95_c = log(oddsratio95_c)
...
*** figure 5 and appendix figure 5
	...
	foreach interval in 1 {
		...
		************************************************************************
		...
		gen log_outflow95 = ln(outflow95_c)

		local depvar = "log_oddsratio95_c "
	...
	graph combine D_ATR_p99_unrev5_dummy1pp D_CIT_unrev5_dummy, col(2) imargin(1 1 10 10) graphregion(fcolor(white) lcolor(white))
	...
	*note("next 5 years).", suffix span)
	...
	graph export "${mw_resultdir}/figure5_company.pdf", as(pdf) replace;

	
		#delimit ; 
	graph combine D_I_credit_unrev5_dummy D_RD_credit_unrev5_dummy, imargin(0 0 10 10) 	graphregion(fcolor(white) lcolor(white))
	title("Outmigration Before and After Tax Change Event") ;
	*note("Notes: The dashed black line indicates the average coefficient over the pre-treatment period. We use", span)
	*note("a balanced panel from 5 years before event to 10 years after. The graph plots {&beta}{sup:h} from the regressions: ", suffix span)
	*note("lnOR{sub:o,d,t+h} {&minus} lnOR{sub:o,d,t} = {&beta}{sup:h}D{sub:o,d,t} + {&epsilon}{sub:o,d,t} , where lnOR is the outmigration log odds-ratio. D{sub:o,d,t} is an event", suffix span)
	...
	*note("change. Only permanent tax changes are included (defined as changes that are not reversed in the", suffix span)
	*note("next 5 years).", suffix span)
	graph export "${mw_resultdir}/appendix_figure5_company.pdf", as(pdf) replace;
