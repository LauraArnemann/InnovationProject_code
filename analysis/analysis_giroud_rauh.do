...
ppmlhdfe  `var' rd_credit total_rd_credit pit cit  if year>=1992   , absorb(assignee_id year) cl(fips_state)
est sto reg3
estadd local yearfe "\checkmark", replace
estadd local firmfe "\checkmark", replace

ppmlhdfe   `var' rd_credit total_rd_credit pit cit total_pit total_cit  if year>=1992  , absorb(assignee_id year) cl(fips_state)
est sto reg4
estadd local yearfe "\checkmark", replace
estadd local firmfe "\checkmark", replace
