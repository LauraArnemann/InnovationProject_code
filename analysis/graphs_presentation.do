...
ppmlhdfe `var' 1.max_treated#1.post_tr pit cit ln_gdp unemployment if inrange(year, 1988, 2018), absorb(estab#event year#event) cl(fips_state#event)
	outreg2 using "$OVERLEAF/tables/stacked_rd_credit_incr_`var'", ///
...
ppmlhdfe `var' 1.max_treated#1.post_tr pit cit ln_gdp unemployment if inrange(year, 1988, 2018) & rd_credit_other!=., absorb(estab#event year#event) cl(fips_state#event)
	outreg2 using "$OVERLEAF/tables/stacked_rd_credit_incr_`var'", ///
