...
foreach direction in $direction {
	...
	*other_weighted3 
	foreach explaining in `weighting_strategy' {
		...
		use "${TEMP}/final_cz_stacked_other_total_`weighting_strategy'_`direction'.dta", replace 
		merge m:1 estab_id year using "${TEMP}/final_cz_stacked_other_zeros.dta", nogen keep(3)
		...
		gen ln_gdp = log(other_gdp_`explaining')
		...
		foreach i of numlist 10 {
			...
			*Event studies
			foreach outc in $outcome {
				...
				ppmlhdfe `outc' f4_binary f3_binary f2_binary zero_1 l?_binary `sample`i'', absorb(estab_id#event year#event#fips_state) cl(estab_id#event)
					est sto inventorreg1
					coefplot inventorreg1, vertical levels(95) recast(connected) omitted graphregion(color(white)) ///
					xline(4.5, lpattern(dash) lwidth(thin) lcolor(black)) keep(f?_binary zero_1 l?_binary) ///
					yline(0, lcolor(red) lwidth(thin)) ylabel(,labsize(medlarge)) ///
					xtitle("Years since Change") graphregion(color(white))
				capture noisily graph export "${RESULTS}/eventstudies/stackedregression/czone/stacked_other_`outc'_`explaining'_`direction'_c1_sample`i'_stateyear_cz.png", replace  
				
				ppmlhdfe `outc' f4_binary f3_binary f2_binary zero_1 l?_binary `sample`i'', absorb(estab_id#event year#event#czone) cl(estab_id#event)
					est sto inventorreg1
					coefplot inventorreg1, vertical levels(95) recast(connected) omitted graphregion(color(white)) ///
					xline(4.5, lpattern(dash) lwidth(thin) lcolor(black)) keep(f?_binary zero_1 l?_binary) ///
					yline(0, lcolor(red) lwidth(thin)) ylabel(,labsize(medlarge)) ///
					xtitle("Years since Change") graphregion(color(white))
				capture noisily graph export "${RESULTS}/eventstudies/stackedregression/czone/stacked_other_`outc'_`explaining'_`direction'_c1b_sample`i'_stateyear_cz.png", replace 
									
				ppmlhdfe `outc' f4_binary f3_binary f2_binary zero_1 l?_binary `other_controls' `sample`i'', absorb(estab_id#event year#event#fips_state) cl(estab_id#event)
					est sto inventorreg3
					coefplot inventorreg3, vertical levels(95) recast(connected) omitted graphregion(color(white)) ///
					xline(4.5, lpattern(dash) lwidth(thin) lcolor(black)) keep(f?_binary zero_1 l?_binary) ///
					yline(0, lcolor(red) lwidth(thin)) ylabel(,labsize(medlarge)) ///
					 xtitle("Years since Change") graphregion(color(white))
				capture noisily graph export "${RESULTS}/eventstudies/stackedregression/czone/stacked_other_`outc'_`explaining'_`direction'_c2_sample`i'_stateyear_cz.png", replace  
				
				ppmlhdfe `outc' f4_binary f3_binary f2_binary zero_1 l?_binary `other_controls' `sample`i'', absorb(estab_id#event year#event#czone) cl(estab_id#event)
					est sto inventorreg3
					coefplot inventorreg3, vertical levels(95) recast(connected) omitted graphregion(color(white)) ///
					xline(4.5, lpattern(dash) lwidth(thin) lcolor(black)) keep(f?_binary zero_1 l?_binary) ///
					yline(0, lcolor(red) lwidth(thin)) ylabel(,labsize(medlarge)) ///
					 xtitle("Years since Change") graphregion(color(white))
				capture noisily graph export "${RESULTS}/eventstudies/stackedregression/czone/stacked_other_`outc'_`explaining'_`direction'_c2b_sample`i'_stateyear_cz.png", replace  
				}
					
			* Also run the logarithm to give comparability with chaisemartin estimator 
			foreach outc in $outcome_log {	
			
				reghdfe `outc' f4_binary f3_binary f2_binary zero_1 l?_binary `sample`i'', absorb(estab_id#event year#event#fips_state) cl(estab_id#event)
					est sto inventorreg3
					coefplot inventorreg3, vertical levels(95) recast(connected) omitted graphregion(color(white)) ///
					xline(4.5, lpattern(dash) lwidth(thin) lcolor(black)) keep(f?_binary zero_1 l?_binary) ///
					yline(0, lcolor(red) lwidth(thin)) ylabel(,labsize(medlarge)) ///
					xtitle("Years since Change") graphregion(color(white))
				capture noisily graph export "${RESULTS}/eventstudies/stackedregression/czone/stacked_other_`outc'_`explaining'_`direction'_c3_sample`i'_stateyear_cz.png", replace 
				
				reghdfe `outc' f4_binary f3_binary f2_binary zero_1 l?_binary `sample`i'', absorb(estab_id#event year#event#czone) cl(estab_id#event)
					est sto inventorreg3
					coefplot inventorreg3, vertical levels(95) recast(connected) omitted graphregion(color(white)) ///
					xline(4.5, lpattern(dash) lwidth(thin) lcolor(black)) keep(f?_binary zero_1 l?_binary) ///
					yline(0, lcolor(red) lwidth(thin)) ylabel(,labsize(medlarge)) ///
					xtitle("Years since Change") graphregion(color(white))
				capture noisily graph export "${RESULTS}/eventstudies/stackedregression/czone/stacked_other_`outc'_`explaining'_`direction'_c3b_sample`i'_stateyear_cz.png", replace 
				
				 reghdfe `outc' f4_binary f3_binary f2_binary zero_1 l?_binary `other_controls' `sample`i'', absorb(estab_id#event year#event#fips_state) cl(estab_id#event)
					est sto inventorreg3
					coefplot inventorreg3, vertical levels(95) recast(connected) omitted graphregion(color(white)) ///
					xline(4.5, lpattern(dash) lwidth(thin) lcolor(black)) keep(f?_binary zero_1 l?_binary) ///
					yline(0, lcolor(red) lwidth(thin)) ylabel(,labsize(medlarge)) ///
					xtitle("Years since Change") graphregion(color(white))
				capture noisily graph export "${RESULTS}/eventstudies/stackedregression/czone/stacked_other_`outc'_`explaining'_`direction'_c4_sample`i'_stateyear_cz.png", replace
				
				reghdfe `outc' f4_binary f3_binary f2_binary zero_1 l?_binary `other_controls' `sample`i'', absorb(estab_id#event year#event#czone) cl(estab_id#event)
					est sto inventorreg3
					coefplot inventorreg3, vertical levels(95) recast(connected) omitted graphregion(color(white)) ///
					xline(4.5, lpattern(dash) lwidth(thin) lcolor(black)) keep(f?_binary zero_1 l?_binary) ///
					yline(0, lcolor(red) lwidth(thin)) ylabel(,labsize(medlarge)) ///
					xtitle("Years since Change") graphregion(color(white))
				capture noisily graph export "${RESULTS}/eventstudies/stackedregression/czone/stacked_other_`outc'_`explaining'_`direction'_c4b_sample`i'_stateyear_cz.png", replace 
