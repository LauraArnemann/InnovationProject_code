...
/*
use "${TEMP}/state_data_cleaned.dta", clear 

xtset fips_state app_year 

gen change_credit = d.rd_credit
*/
...
bysort app_year assignee_id: gen count = _N 
gen multistate_firm = 0 
replace multistate_firm = 1 if count>1 
bysort assignee_id: egen max_multistate = max(multistate_firm)
...
foreach fips in CA PA TX {
	...
	* Exporting the graph 
	...
	title("Case Study: `fips' ") xtitle("Years since Change") graphregion(color(white))
