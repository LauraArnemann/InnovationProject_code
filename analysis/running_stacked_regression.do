	...
	foreach indepvar in `indepvar' {
		...
		* Set different sample restrictions as well 
		...
		forvalues i =1/7 {
			...
			*Event studies
			ppmlhdfe `outc' f4_binary f3_binary f2_binary zero_1 l?_binary `sample`i'', absorb(estab#event year#event) cl(fips_state#event)
			...
			title("`indepvar', `direction' - no controls") xtitle("Years since Change") graphregion(color(white))
				graph export "${RESULTS}/graphs/`outc'/stacked_`indepvar'_`direction'_sample`i'.png", replace  
		
			ppmlhdfe `outc' f4_binary f3_binary f2_binary zero_1 l?_binary $controls `sample`i'', absorb(estab#event year#event) cl(fips_state#event)
			...
			title("`indepvar', `direction' - controls") xtitle("Years since Change") graphregion(color(white))
				graph export "${RESULTS}/graphs/`outc'/stacked_`indepvar'_`direction'_c1_sample`i'.png", replace  
				
			ppmlhdfe `outc' f4_binary f3_binary f2_binary zero_1 l?_binary $controls $controls_other `sample`i'', absorb(estab#event year#event) cl(fips_state#event)
			...
			title("`indepvar', `direction' - controls (incl other)") xtitle("Years since Change") graphregion(color(white))
				graph export "${RESULTS}/graphs/`outc'/stacked_`indepvar'_`direction'_c2_sample`i'.png", replace  
			
			ppmlhdfe `outc' f4_binary f3_binary f2_binary zero_1 l?_binary $controls2 $controls_other2 `sample`i'', absorb(estab#event year#event) cl(fips_state#event)
			...
			title("`indepvar', `direction' - controls (incl other , incl gdp unemployment)") xtitle("Years since Change") graphregion(color(white))
				graph export "${RESULTS}/graphs/`outc'/stacked_`indepvar'_`direction'_c3_sample`i'.png", replace  
				
			*DiD
			ppmlhdfe `outc' max_treated##post_tr `sample`i'', absorb(estab#event year#event) cl(fips_state#event)
				outreg2 using "$RESULTS/tables/`outc'/change_stack_zero_`indepvar'_`direction'_sample`i'", ///
			...
			forvalues i =1/5 {
				...
				*Event studies
				...
				title("`indepvar', `var2', `direction' - no controls") xtitle("Years since Change") graphregion(color(white))
					graph export "${RESULTS}/graphs/`outc'/stacked_other_`var2'_`direction'_sample`i'.png", replace  
				...
				title("`indepvar', `var2', `direction' - controls") xtitle("Years since Change") graphregion(color(white))
					graph export "${RESULTS}/graphs/`outc'/stacked_other_`var2'_`direction'_c1_sample`i'.png", replace  
				...
				title("`indepvar', `var2', `direction' - controls (incl other)") xtitle("Years since Change") graphregion(color(white))
					graph export "${RESULTS}/graphs/`outc'/stacked_other_`var2'_`direction'_c2_sample`i'.png", replace  
				...
				title("`indepvar', `var2', `direction' - controls (incl other, incl gdp unemployment)") xtitle("Years since Change") graphregion(color(white))
					graph export "${RESULTS}/graphs/`outc'/stacked_other_`var2'_`direction'_c3_sample`i'.png", replace 
						
				*DiD
				ppmlhdfe `outc' max_treated##post_tr `sample`i'', absorb(estab#event year#event) cl(fips_state#event)
					outreg2 using "$RESULTS/tables/`outc'/change_stack_other_zero_`var2'_`direction'_sample`i'", ///
					replace dec(3) stats(coef se) addstat(Pseudo R2, e(r2_p)) noni nodepvar tex(frag) ///
					ctitle("`outc'") lab
